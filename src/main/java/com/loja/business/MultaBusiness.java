...
public class MultaBusiness implements IMultaBusiness{
    ...
    public void aplicar(ContratoAluguel contrato){
        if (contrato == null){
            throw new MultaException("Não é possível aplicar multa para um contrato nulo.");
        }

        BigDecimal valorAtraso = calcularAtraso(contrato);

        if (valorAtraso.compareTo(BigDecimal.ZERO) <= 0){
            throw new MultaException("Este contrato não possui dias de atraso para aplicação da multa.");
        }
    ...
    public void quitar(String multaId){
        if (multaId == null || multaId.trim().isEmpty()){
            throw new MultaException("ID da multa inválido para operação de quitação.");
        }
        Multa existente = multaRepository.buscar(multaId);

        if (existente == null){
            throw new MultaException("Multa não encontrada para o ID: " + multaId);
        }
    ...
    public Map<String, Multa> listarPorCliente(String clienteId) {
        if (clienteId == null || clienteId.trim().isEmpty()) {
            throw new MultaException("ID do cliente inválido para a consulta.");
        }
    ...
    public void atualizar(Multa multa){
        if (multa == null){
            throw new MultaException("Multa inválida para atualização!");
        }
    ...
    public void deletarMulta(String id) {
        if (id == null || id.trim().isEmpty()) {
            throw new MultaException("ID inválido para exclusão da multa.");
        }

        boolean deletado = multaRepository.deletar(id);

        if (!deletado) {
            throw new MultaException("Não foi possível deletar: Multa não encontrada com o ID: " + id);
        }
    }
