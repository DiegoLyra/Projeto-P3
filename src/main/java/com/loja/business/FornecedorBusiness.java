...
import com.loja.exceptions.FornecedorException;
...
public class FornecedorBusiness implements IFornecedorBusiness {
    ...
    public void cadastrar(Fornecedor f){
        if (f == null){ // se o objeto foi preenchido
            throw new FornecedorException("Não foi possivel cadastrar o objeto fornecedor!");
        } else if (repo.buscar(f.getId()) != null) { // se ja existe com o id
            throw new FornecedorException("Já existe outro fornecedor com esse ID: " + repo.buscar(f.getId()).getNome());
        } else if (f.getNome() == null || f.getNome().isBlank()) { // se o nome tá ok
            throw new FornecedorException("Não foi possível cadastrar o fornecedor: Falha no nome");
        } else if (f.getId() == null || f.getId().isBlank()) { // se o id tá ok
            throw new FornecedorException("Não foi possível cadastrar o fornecedor: Falha no ID");
        }
        ...
        if (nomeExiste) { // se já existe com o nome
            throw new FornecedorException("Já existe um fornecedor com o nome: " + f.getNome());
        }
    ...
    public Fornecedor buscar(String id){
        if (id == null){
            throw new FornecedorException("Id do fornecedor inválido");
        }
    ...
    public void atualizar(Fornecedor fornecedor){
        if (fornecedor == null){
            throw new FornecedorException("Fornecedor inválido!");
        } else if (fornecedor.getNome() == null || fornecedor.getNome().isBlank()){
            throw new FornecedorException("Nome do fornecedor inválido!");
        }
        if (!repo.atualizar(fornecedor)){
            throw new FornecedorException("Não foi possível atualizar!");
        }
    }

    public void deletar(String id){
        if (repo.buscar(id) == null || id.isBlank()){
            throw new FornecedorException("Fornecedor não encontrado: " + id);
        } else if (repo.buscar(id).hasHistorico()){
            throw new FornecedorException("O fornecedor não pode ser excluído, tem histórico");
        } else if (repo.listar().values().stream().noneMatch(forn -> forn.getId().equals(id))){
            throw new FornecedorException("Não existe um fornecedor com esse id");
        }
        repo.deletar(id);
    }
