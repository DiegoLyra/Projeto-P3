...
import com.loja.exceptions.CategoriaException;
...
public class CategoriaBusiness implements ICategoriaBusiness {
    ...
    public void cadastrar(Categoria c){
        if (c == null){ // se o objeto foi preenchido
            throw new CategoriaException("Não foi possivel cadastrar o objeto categoria!");
        } else if (repo.buscar(c.getId()) != null) { // se ja existe com o id
            throw new CategoriaException("Já existe outra categoria com esse ID: " + repo.buscar(c.getId()).getNome());
        } else if (c.getNome() == null || c.getNome().isBlank()) { // se o nome tá ok
            throw new CategoriaException("Não foi possível cadastrar a categoria: Falha no nome");
        } else if (c.getId() == null || c.getId().isBlank()) { // se o id tá ok
            throw new CategoriaException("Não foi possível cadastrar a categoria: Falha no ID");
        }
        ...
        if (nomeExiste) { // se já existe com o nome
            throw new CategoriaException("Já existe uma categoria com o nome: " + c.getNome());
        }
    ...
    public void atualizar(Categoria categoria){
        if (categoria == null){
            ...
            throw new CategoriaException("Nome da categoria inválido!");
        }
        if (!repo.atualizar(categoria)){
            throw new CategoriaException("Não foi possível atualizar!");
        }
    }
    ...
    public void deletar(String id){
        if (repo.buscar(id) == null || id.isBlank()){
            throw new CategoriaException("Categoria não encontrada: " + id);
        } else if (repo.buscar(id).hasHistorico()){
            throw new CategoriaException("A categoria não pode ser excluida, tem histórico");
        } else if (repo.listar().values().stream().noneMatch(cat -> cat.getId().equals(id))){
            throw new CategoriaException("Não existe uma categoria com esse id");
        }
        repo.deletar(id);
    }
